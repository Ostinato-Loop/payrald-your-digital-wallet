...
function SignUp() {
  ...
  const [form, setForm] = useState({ name: "", handle: "", email: "", phone: "" });
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!valid) return;
    setLoading(true);
    setTimeout(() => navigate({ to: "/verify" }), 800);
  };
  ...
  return (
    <div className="flex flex-1 flex-col px-6 pb-8 pt-4">
      ...
      <div className="mt-6">
        <h1 className="text-2xl font-semibold tracking-tight">Create your PayRald</h1>
      ...
      <form onSubmit={submit} className="mt-7 flex flex-col gap-3">
        ...
        <Field
          ...
          label="Username"
          value={form.handle}
          onChange={(v) => setForm({ ...form, handle: v.replace(/[^a-z0-9_]/gi, "").toLowerCase() })}
          placeholder="boyd"
          prefix="@"
        />
        <Field
        ...
        <label className="mt-1 flex items-start gap-2 text-xs text-muted-foreground">
          ...
          />
          <span>
            I agree to PayRald's <span className="text-primary">Terms</span> and{" "}
            <span className="text-primary">Privacy Policy</span>. ALIA may verify my identity.
          </span>
        </label>
